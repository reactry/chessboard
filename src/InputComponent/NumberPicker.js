...
export default function NumberPicker ({
	...
	let numberItems = numbers.map((v, i) => {
		...
		let classNames = `inline-block px-2 py-1 -my-1 cursor-pointer text-sm font-bold border-y-4 ${extraClass}`;
		return (
	...
	return (
		<div className="NumberPicker mb-4">
			<h2 className="text-sm font-bold mb-1">{title}</h2>
			<div className="outline-none p-1 focus:bg-slate-400" tabIndex="0" onKeyDown={handleKeyDown}>
				{numberItems}
			</div>
		</div>
	);
